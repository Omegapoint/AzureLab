...
namespace Ginastics.IntegrationTests
{
    public class GinInformationTest
    {
        ...
        public void ShouldDeleteGin()
        {
            ...
            var deleteRequest = new RestRequest(new Uri($"{Host}/gin/{gin.GinId}"), Method.DELETE, DataFormat.Json);
            var deleteResponse = _httpClient.Delete(deleteRequest);
            deleteResponse.StatusCode.Should().Be(HttpStatusCode.NoContent);

            var listGinRequest = new RestRequest(new Uri($"{Host}/gin"), Method.GET, DataFormat.Json);
            var listResponse = _httpClient.Get<AllGins>(listGinRequest);

            listResponse.Data.Gins.Should().NotContain(gin);
        }
